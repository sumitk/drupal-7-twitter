...
class Twitter {
   ...
   */
  public function auth_request($url, $params = array(), $method = 'GET', $multipart = FALSE) {
    // Multipart form data parameters (uploads) do not need to be signed in
    ...
    try {
      switch ($method) {
        ...
        case 'POST':
          $params = array_merge($params, $request->get_parameters());
          return $this->request($request->get_normalized_http_url(), $params, 'POST', $multipart);
      }
    }
    catch (TwitterException $e) {
      watchdog('twitter', '!message', array('!message' => $e->__toString()), WATCHDOG_ERROR);
   ...
   */
  protected function request($url, $params = array(), $method = 'GET', $multipart = FALSE) {
    $headers = array();
    $boundary = '';
    ...
    $data = $this->process_request_params($url, $headers, $params, $method, $boundary);
  ...
  /**
    * Process the parameters for a request to the Twitter API.
    *
    * @param $url
    *    The to be modified request url.
    * @param $headers
    *    The to be modified headers.
    * @param array $params
    *    The request parameters.
    * @param string $method
    *    The request method.
    * @param string $boundary
    *    The unique boundary string.
    *
    * @return string
    *    The request body.
    */
  protected function process_request_params(&$url, &$headers, $params, $method, $boundary) {
    if (!count($params)) {
      return '';
    }
    ...
    foreach ($params as $key => $val) {
      ...
      $oauth_params[] = $key .'="' . urlencode($val) . '"';
      unset($params[$key]);
    }

    // For multipart, oauth params get tagged onto the authorization
    // header.
    $headers['Authorization'] .= ' ' . implode(", " ,$oauth_params);

    return $this->encode_multipart_data($boundary, $params);
  }
   ...
   *   The encoded data.
   */
  protected function encode_multipart_data($boundary, array $params) {
    // $params = array(
    //   'filename' => '',
    //   'filemime' => '',
    //   'media' => '',
    ...
    $data = "--$boundary\r\n";
    $data .= "Content-Disposition: form-data; name=\"media\"; filename=\"" . $params['filename'] . "\"\r\n";
    $data .= "Content-Type: " . $params['filemime'] . "\r\n\r\n";
    $data .= $params['media'] . "\r\n";
    $data .= "--$boundary--";
   ...
   */
  protected function create_url($path, $format = '.json', $multipart = FALSE) {
    if ($multipart) {
      $url = variable_get('twitter_upload', TWITTER_UPLOAD);
    }
    else {
      $url = variable_get('twitter_api', TWITTER_API);
    }

    $url .= '/' . variable_get('twitter_api_version', TWITTER_API_VERSION) . '/' . $path . $format;
    return $url;
  }
   ...
   ***********************************************/
  /**
   * Upload media (images) to use in a Tweet or Twitter-hosted Card.
   *
   ...
   */
  public function media_upload($filename, $filemime, $media) {
    $params = array();
    $params['filename'] = $filename;
    $params['filemime'] = $filemime;
    $params['media'] = $media;
    return $this->call('media/upload', $params, 'POST', TRUE);
  }
   ...
   */
  public function call($path, $params = array(), $method = 'GET', $multipart = FALSE) {
    $url = $this->create_url($path, '.json', $multipart);

    try {
      $response = $this->auth_request($url, $params, $method, $multipart);
    }
    catch (TwitterException $e) {
      watchdog('twitter', '!message', array('!message' => $e->__toString()), WATCHDOG_ERROR);
      throw $e;
